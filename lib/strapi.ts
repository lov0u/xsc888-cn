/**
 * Strapi API 客户端
 * 对接 strapi.ra0.cn 后端，site = xsc888
 */

const STRAPI_URL =
  process.env.NEXT_PUBLIC_STRAPI_URL || "http://strapi.ra0.cn";
...
export async function getArticles(
  ...
  try {
    const url = `${STRAPI_URL}/api/articles?fields[0]=title&fields[1]=slug&fields[2]=excerpt&fields[3]=publishedAt&populate[coverImage][fields][0]=url&pagination[page]=${page}&pagination[pageSize]=${pageSize}&sort[0]=publishedAt:desc&filters[site][slug][$eq]=xsc888`;
...
export async function getArticle(slug: string): Promise<Article | null> {
  try {
    const url = `${STRAPI_URL}/api/articles?filters[slug][$eq]=${slug}&filters[site][slug][$eq]=xsc888&populate=*`;
...
export async function getTags(): Promise<Tag[]> {
  try {
    const url = `${STRAPI_URL}/api/tags?filters[articles][site][slug][$eq]=xsc888&fields[0]=name&fields[1]=slug&pagination[pageSize]=100`;
...
export async function getArticlesByTag(
  ...
  try {
    ...
    const url = `${STRAPI_URL}/api/articles?fields[0]=title&fields[1]=slug&fields[2]=excerpt&fields[3]=publishedAt&populate[coverImage][fields][0]=url&pagination[page]=${page}&pagination[pageSize]=${pageSize}&sort[0]=publishedAt:desc&filters[site][slug][$eq]=xsc888&filters[tags][slug][$eq]=${tagSlug}`;
